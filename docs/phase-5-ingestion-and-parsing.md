# Architectural Design Document: Phase 5 — Local Ingestion & Parsing

This document establishes the architectural layout and technical specifications for **Phase 5: Local Ingestion & Parsing** within the Academic Paper Discovery and Synthesis Engine.
...
2. **Layout-Aware Parsing:** The PDF is processed through Docling to reconstruct its hierarchical academic structure (sections, paragraphs, tables).
...
5. **Vector Storage:** The validated claims are embedded and stored in Qdrant, utilizing extensive payload indexing for high-speed downstream filtering.
...
### 2.1. The Layout-Aware Parser (Docling)

Instead of flattening the document into a single string, the system utilizes Docling to perform deep layout analysis.

* **Reading-Order Resolution:** Docling's underlying models (like DocLayNet) natively understand two-column academic formats and stitch sentences together correctly across columns and page breaks.
* **Table Reconstruction (TableFormer):** Tables are not converted to messy inline text. They are detected, reconstructed as grid objects, and parsed into structured Markdown or HTML tables, preserving the relationships between column headers and data rows.
...
* **Structural Slicing:** This chunker only breaks text at explicit Docling nodes (e.g., `Paragraph`, `List Item`, `Table`).
...
* **Corrupted Chunk Handling:** If a PDF is poorly scanned and Docling outputs garbage text (e.g., overlapping font glyphs), the LLM will likely fail the `source_quote` substring match. The system logs the chunk as `UNPARSABLE` and moves on, prioritizing high-confidence data over noisy data.
