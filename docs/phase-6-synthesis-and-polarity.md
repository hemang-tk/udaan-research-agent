...
- **Language:** Python service (FastAPI) — **scikit-learn** clustering (CPU-bound), running on the Hugging Face CPU container; no hosted equivalent, so this stage is the same on main and `local-infra`.
- **Polarity-judge LLM** (behind the `LLM_PROVIDER` interface; trust-critical): hosted APIs — free → **Groq Llama 3.3 70B**; recommended → **Claude Opus 4.8** (use adaptive thinking; omit `temperature`/`top_p`). The self-hosted variant (Ollama-served Qwen2.5) lives on the `local-infra` branch.
- **Execution note:** the O(n²) clustering is CPU-bound and runs in-process inside the single container; the polarity LLM is a hosted API call, so the two never contend for the same hardware.
...
The architecture splits the compute load: statistical clustering runs as CPU-bound scikit-learn execution on the container, while polarity evaluation is delegated to a hosted LLM API — so the two never contend for the same hardware.

* **Algorithm Selection:** We utilize **Agglomerative Hierarchical Clustering** (scikit-learn) with Cosine distance. Unlike K-Means, Agglomerative clustering does not require us to pre-determine the number of clusters ($k$).
...
### 2.3. Polarity Inference Worker Pool (Hosted LLM)

Once clustered, the claims are reconstructed into text and passed to the hosted polarity-judge LLM.
...
* **Batching Constraints:** To stay within the model's context window and provider rate limits, clusters containing more than 15 claims are sub-divided or summarized via a map-reduce chain before final polarity evaluation.
...
### 4.2. Resource Separation (CPU clustering, hosted inference)

Because this phase pairs heavy local clustering with LLM evaluation, the two workloads are kept on separate resources.

* Clusters are submitted to the hosted polarity LLM with bounded concurrency so a burst of clusters never trips the provider's rate limit.
* The heavy $O(n^2)$ distance calculations run on the container CPU via scikit-learn, while the polarity model runs on the provider's infrastructure — so the local container's memory stays bounded regardless of model size.
...
| --- | --- | --- |
| **Clustering Execution Time** | $\le 1.5\text{s}$ for 1000 vectors | scikit-learn Profiler |
...
| **Max Tokens per Inference** | Bounded to model context limits | Provider Usage Telemetry |
