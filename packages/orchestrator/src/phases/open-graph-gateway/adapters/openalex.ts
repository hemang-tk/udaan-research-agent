...
export class OpenAlexAdapter implements OpenGraphProvider {
  ...
  // perPage=50 (not 200) keeps the response ~1MB so it downloads well under the
  // per-provider timeout on a slow link; re-ranking only needs the top slice.
  constructor(private readonly baseUrl = "https://api.openalex.org", private readonly perPage = 50) {}
